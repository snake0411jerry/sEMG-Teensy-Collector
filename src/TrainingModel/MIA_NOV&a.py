...
# ==========================================
data_dir = r"D:\project\NCU\114\Jumior\AI Project\Dataset\0602\Combined"
...
# ==========================================
# 3.5 訓練歷史視覺化 (Learning Curve) - 仿 Screenshot 2026-06-12 100106.jpg 風格
# ==========================================
print("\n📊 繪製模型訓練歷史曲線...")
...
# 定義要繪製的指標對應 (Keras 多任務會自動產生這些 key)
# 結構: (history 中的 key, 圖表標題)
metrics_to_plot = [
    ('loss', 'Total Loss'),                         # 總損失
    ('out_emg_loss', 'EMG Loss (MSE)'),             # 任務 1: 肌電圖損失
    ('out_comp_loss', 'Compensation Loss (BCE)'),   # 任務 2: 代償分類損失
    ('out_emg_mae', 'EMG MAE'),                     # 任務 1: 肌電圖平均絕對誤差
    ('out_comp_accuracy', 'Compensation Accuracy')  # 任務 2: 代償分類準確率
]

# 定義指數移動平均 (EMA) 平滑函數，用來畫出圖中 smooth 的虛線
...
for i, (metric, title) in enumerate(metrics_to_plot):
    ...
    # 讀取 Training 與 Validation 數據
    train_data = hist[metric]
    val_data = hist[f'val_{metric}']
    
    # 1. 繪製原始數據 (帶有 marker 的實線，稍作透明處理)
...
# 因為我們只有 5 個指標，第 6 個格子會是空的，將其隱藏
axes[5].axis('off')

plt.tight_layout()
plt.subplots_adjust(top=0.92) # 預留空間給大標題
plt.show()
# ==========================================
...
# 5. 儲存模型與正規化器 
# ==========================================
print("\n💾 正在儲存多任務模型 (實驗 B: 無動力學輸入)...")
model.save("teacher_multitask_random_noVA.keras")
joblib.dump(scaler_x, "scaler_x_multitask_random_noVA.pkl")
joblib.dump(scaler_y_emg, "scaler_y_emg_multitask_random.pkl") # Y的Scaler不受影響，但存個新檔比較保險
print("✅ 實驗 B 模型儲存完畢！")